...
2. 「Outlook から抽出」タブで:
   ...
   - 子フォルダがあるフォルダには `+子フォルダ` ボタンが表示される — クリックで子フォルダをまとめて選択/解除
   - 期間を指定
   - 「抽出 & 分析する」を押す
3. ネットワーク可視化が表示される
...
4. Outlook フォルダを選択（番号の後に `+` を付けると子フォルダも含む。例: `3+`）→ 自動でメール抽出 & サーバーにアップロード
5. ブラウザに分析結果が自動表示される
...
親フォルダだけでなく、子フォルダをまとめて選択する機能をサポート。

| 操作 | Web UI | CLI（.bat 実行時） |
|------|--------|-------------------|
| 個別選択 | チェックボックスをクリック | 番号を入力（例: `3`） |
| 子フォルダ込み | `+子フォルダ` ボタンをクリック | 番号に `+` を付ける（例: `3+`） |
| 混在 | 自由にチェック | カンマ区切り（例: `1,3+,7`） |

> 例: `客先名A/` の下に `顧客A`, `顧客B`, `顧客C` がある場合、`客先名A/` の `+子フォルダ` を押すと3つの子フォルダがすべて選択される。`客先名A/` 自体のチェックボックスは独立して動作する。
