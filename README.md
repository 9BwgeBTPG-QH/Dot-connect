...
1. 共有フォルダに Dot-connect を配置
2. `setup.bat` → `start.bat` を実行（サーバー上で常時起動）
3. `config.yaml` の `network_share_path` を共有パスに設定（例: `\\SERVER\Dot-connect`）
...
├── python\                  ← 埋め込みPython（setup.batで生成）
└── ...
```

> **注意**: ファイルサーバーの Windows Firewall でポート 8000 の受信を許可する必要がある場合があります。
...
| Outlook COM 制限 | M365 C2R 環境では COM が使えないため、ローカル抽出ツール方式で回避（方法B） |
